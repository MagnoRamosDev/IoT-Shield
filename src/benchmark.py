...
from src.dashboard import print_table, print_ui
...
def run_benchmark(out_dir="results", samples=10000):
    ...
    if (
        ...
        or not os.path.exists(c_path)
        or not os.path.exists(test_csv)
    ):
        print_ui(
            f"[bold red][!] Benchmark requires trained model, C code, and data file ({test_csv}).[/bold red]"
        )
        return

    print_ui(
        "[bold cyan][*] Compiling C model to shared library for benchmarking...[/bold cyan]"
    )
...
""")
    ...
    concurrent_features = [
        "src2dst_concurrent_flows",
        "dst2src_concurrent_flows",
        "bidirectional_concurrent_flows",
    ]
    ...
    table = Table(
        title="Inference Benchmark (Python vs Native C)",
        show_header=True,
...
""")
        ...
        time_e2e_emu, cpu_emu, ram_emu = run_with_time(
            emulated_cmd + [c_sniffer, bench_pcap]
        )
        ...
        e2e_table = Table(
            title="End-to-End Pipeline (Sniffer + Inference)",
            show_header=True,
            header_style="bold yellow",
        )
